...
export async function getPhoenixDatabase() {
    ...
    if (!indexPromise) {
        indexPromise = Promise.all([
            ...
            database.collection('thermalRuns').createIndex({ userId: 1, projectId: 1, createdAt: -1 }),
            database.collection('screeningRuns').createIndex({ userId: 1, projectId: 1, createdAt: -1 }),
        ]);
    }
